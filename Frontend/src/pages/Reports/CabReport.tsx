...
export default function CabReport() {
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      statusesToFetch.forEach(st => {
        requests.push(dashboard.getCabReport(st, session.empCode, formattedFrom, formattedTo));
      });
      ...
      results.forEach((res) => {
        if (res.status === "fulfilled" && res.value?.success && Array.isArray(res.value.data)) {
          res.value.data.forEach((d: any) => {
            data.push({ ...d, _type: 'CAB_CRF', idDisplay: d.crfId });
          });
        }
      });
  ...
  return (
    <div className={`min-h-screen theme-transition font-sans flex flex-col ${isLight ? 'bg-slate-50 text-slate-800' : 'bg-[#0F172A] text-slate-200'}`}>
      <div className="fixed top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#0084FF]/10 via-transparent to-transparent pointer-events-none z-0"></div>
      
      {/* Header matching CabDetails */}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-[#0084FF]/10 flex items-center justify-center border border-[#0084FF]/20">
                <svg className="w-4 h-4 text-[#0084FF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
              </div>
              <div>
                <h1 className={`text-lg font-bold tracking-wide leading-tight ${isLight ? 'text-gray-900' : 'text-white'}`}>CAB Reports</h1>
                <p className="text-[10px] text-[#0084FF] font-mono tracking-wider uppercase">Analytics & History</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
             ...
             <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0084FF] to-purple-600 flex items-center justify-center text-white font-bold shadow-lg">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 py-8 relative z-10 flex flex-col flex-1 gap-6">
        ...
        <div className="glass-panel rounded-2xl p-6 border border-white/10 flex flex-col xl:flex-row gap-6 items-start xl:items-end shrink-0 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#0084FF] via-blue-500 to-transparent opacity-50"></div>
          ...
          <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[1fr_auto] gap-8">
            ...
            <div className="space-y-3">
              <label className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wider ${isLight ? 'text-gray-600' : 'text-gray-400'}`}>
                <svg className="w-4 h-4 text-[#0084FF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                Report Status
              </label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-sm font-bold tracking-wide transition-all ${
                      status === opt.val 
                        ? 'bg-[#0084FF] text-white shadow-[0_0_15px_rgba(0,132,255,0.3)] border border-blue-400' 
                        : isLight
            ...
            <div className="space-y-3">
              <label className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wider ${isLight ? 'text-gray-600' : 'text-gray-400'}`}>
                <svg className="w-4 h-4 text-[#0084FF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                Date Range
              </label>
              <div className={`flex items-center gap-3 p-1.5 rounded-2xl border ${isLight ? 'bg-black/5 border-black/10' : 'bg-black/30 border-white/5'}`}>
                <div className="relative group flex-1">
                   <div className={`absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none group-focus-within:text-[#0084FF] transition-colors ${isLight ? 'text-black' : 'text-gray-500'}`}>
                     From
                   </div>
                   <input type="date" max={toDate || new Date().toISOString().split('T')[0]} value={fromDate} onChange={e => setFromDate(e.target.value)} className={`w-full bg-transparent rounded-xl pl-14 pr-4 py-2 text-sm outline-none focus:bg-white/5 transition-all font-mono ${isLight ? 'text-black' : 'text-gray-200'}`} style={{ colorScheme: isLight ? 'light' : 'dark' }} />
                </div>
                ...
                <div className="relative group flex-1">
                   ...
                   <input type="date" min={fromDate} max={new Date().toISOString().split('T')[0]} value={toDate} onChange={e => setToDate(e.target.value)} className={`w-full bg-transparent rounded-xl pl-10 pr-4 py-2 text-sm outline-none focus:bg-white/5 transition-all font-mono ${isLight ? 'text-black' : 'text-gray-200'}`} style={{ colorScheme: isLight ? 'light' : 'dark' }} />
                </div>
              </div>
            </div>
          ...
          <button onClick={loadReports} className="w-full xl:w-auto flex items-center justify-center gap-3 px-8 py-4 xl:py-3.5 bg-gradient-to-r from-[#0084FF] to-blue-600 hover:from-blue-500 hover:to-blue-700 text-white rounded-xl text-sm font-bold uppercase tracking-wider transition-all shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 hover:-translate-y-0.5">
        ...
        <div className={`flex-1 glass-panel rounded-2xl border overflow-hidden flex flex-col ${isLight ? 'border-black/10' : 'border-white/10'}`}>
          {loading ? (
            <div className="flex-1 flex items-center justify-center py-20">
              <div className="w-8 h-8 rounded-full border-2 border-[#0084FF]/20 border-t-[#0084FF] animate-spin"></div>
            </div>
          ) : reports.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
              <div className="w-20 h-20 rounded-full bg-[#0084FF]/5 flex items-center justify-center mb-6">
                <svg className="w-10 h-10 text-[#0084FF]/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-sm text-gray-400 max-w-md">No CAB tracking data was found matching your filter criteria.</p>
            </div>
          ) : (
                    ...
                    <tr key={`${row._type}-${row.idDisplay}-${i}`} className={`transition-colors ${isLight ? 'hover:bg-black/5' : 'hover:bg-white/5'}`}>
                      <td className="p-4 whitespace-nowrap">
                        ...
                        {(row.req_id || row.reqId) && (
                          <div className="text-[10px] text-[#0084FF] font-mono font-bold tracking-tight mt-0.5">{row.req_id || row.reqId}</div>
                        )}
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        <div className="flex flex-col gap-1">
                          <span className="px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-[#0084FF]/10 text-[#0084FF] border border-[#0084FF]/20 text-center">
                            {row._type === 'CAB_TICKET' ? 'TICKET' : 'CRF'}
                          </span>
                      ...
                      <td className="p-4 min-w-[200px] max-w-[300px]">
                        ...
                        <div className="text-[10px] text-gray-500 font-mono mt-0.5 truncate" title={row.changesToBeMade || ''}>
                          {row.changesToBeMade || ''}
                        </div>
                      </td>
                      <td className="p-4 whitespace-nowrap">
                      ...
                      <td className="p-4 whitespace-nowrap text-right">
                        <button onClick={() => navigate('/Reports/Details', { state: { report: row } })} className="px-3 py-1.5 rounded-lg bg-[#0084FF]/10 border border-[#0084FF]/20 hover:bg-[#0084FF]/20 text-xs font-bold text-[#0084FF] transition-colors">Details</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
