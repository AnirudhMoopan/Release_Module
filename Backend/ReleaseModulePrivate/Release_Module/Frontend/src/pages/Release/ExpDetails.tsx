...
import { crf, expRequest, cabRequest, auth, getDashboardRoute } from "../../services/api";
...
export default function ExpDetails() {
  ...
  const [reasonForExpedite, setReasonForExpedite] = useState("");
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.success && res.data) {
        setSchemas(res.data.map(s => s.schemaName));
      }
    })();
  }, []);
  ...
  const handleCrfSearch = async () => {
    ...
    if (requestType === 'CRF') {
      const res = await crf.getDetails(parseInt(crfId));
      if (res.success && res.data) {
        setCrfData(res.data);
      } else {
      ...
      // TICKET lookup
      const res = await cabRequest.getTicketDetails(parseInt(crfId));
      if (res.success && res.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!crfId || !subject || !changesToBeMade || !reasonForExpedite || !requirementType) {
    ...
    formData.append("ReasonForExpedite", reasonForExpedite);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#0084FF]/20 flex items-center justify-center">
              <svg className="w-4 h-4 text-[#0084FF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
            </div>
            <div>
              <h1 className="text-lg font-semibold text-white tracking-wide">EXP Release Details</h1>
              <p className="text-[10px] text-gray-500 font-mono mt-0.5 uppercase tracking-wider">Expedite Workflow</p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6 flex items-center justify-between">
              ...
              <button 
                type="button" 
                onClick={clearForm}
                className="px-4 py-2 mt-[26px] flex items-center gap-2 rounded-xl text-xs font-bold tracking-widest uppercase transition-all bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white border border-red-500/20 hover:border-red-500 cursor-pointer"
              >
            ...
            {showForm && (
              <div id="unified-form">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
                  ...
                  <div>
                    <label className="field-label">Changes Incorporated *</label>
                ...
                {/* Reason, Requirement Type, Mobile Number */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-5">
                  <div>
                    <label className="field-label">Reason for Expedite *</label>
                    <textarea className="field-input w-full" maxLength={500} rows={2} placeholder="maximum 500 characters" style={{ resize: 'vertical', minHeight: '60px', maxHeight: '150px' }} value={reasonForExpedite} onChange={e => setReasonForExpedite(e.target.value)} required></textarea>
                  </div>
                  <div>
